...
async def fun_group(ctx: context.Context) -> None:
    pass

@fun_group.child
@lightbulb.command("meme", "Get a meme")
@lightbulb.implements(commands.SlashSubCommand, commands.PrefixSubCommand)
async def meme_subcommand(ctx: context.Context) -> None:
    async with ctx.bot.d.aio_session.get(
        "https://meme-api.herokuapp.com/gimme"
    ) as response:
        res = await response.json()

        if response.ok and not res["nsfw"] == True:
            link = res["postLink"]
            title = res["title"]
            img_url = res["url"]
...
@lightbulb.implements(commands.SlashSubCommand, commands.PrefixSubCommand)
async def animal_subcommand(ctx: context.Context) -> None:
    select_menu = (
    ...
    try:
        ...
        if (
            ...
            and inter.user == ctx.author
            and inter.message == msg
            and inter.component_type == hikari.ComponentType.SELECT_MENU
        ):
            animal = inter.values[0]
            async with ctx.bot.d.aio_session.get(
                f"https://some-random-api.ml/animal/{animal}"
            ) as res:
                if res.ok:
                    res = await res.json()
                    embed = hikari.Embed(description=res["fact"])
